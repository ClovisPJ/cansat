...
int main() {
  ...
  for (int t = 1; t < 100; t++) {
    ...
    if (gps_fix()) {
      ...
      gps_get_nmea("$GPGGA");
      exttm = gps_time;
      temp_time = (struct tm){exttm.tm_sec, exttm.tm_min, exttm.tm_hour, exttm.tm_mday, exttm.tm_mon, exttm.tm_year, exttm.tm_wday, exttm.tm_yday};
      gsl_vector_set(temp_location, 0, gsl_vector_get(gps_location, 0) );
      gsl_vector_set(temp_location, 1, gsl_vector_get(gps_location, 1) );
      gsl_vector_set(temp_location, 2, gps_altitude );
    } else {
      printf("No fix\n");
      exit(1);
    }

    timestep = difftime(temp_time, time);
    time = temp_time;
    gsl_vector_sub(location, temp_location);
    gsl_vector_memcpy(velocity, location);
    gsl_vector_memcpy(location, temp_location);

    adxl345_update();
    
    acc = adxl345_getacceleration();
    gsl_vector_set(acceleration, 0, acc[0] * adxl345_getscale() * 9.81);
    gsl_vector_set(acceleration, 1, acc[1] * adxl345_getscale() * 9.81);
    gsl_vector_set(acceleration, 2, acc[2] * adxl345_getscale() * 9.81);
